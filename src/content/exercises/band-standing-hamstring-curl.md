...
contraindications:
  ...
  - Cramping in the hamstring that does not settle
stopIf:
...
strengthening, which is backwards more often than people expect. A hamstring that is short
and weak does not become long by being stretched; it usually needs to be strong through
range.

Keeping the thigh still and letting only the lower leg move is what keeps this at the knee.
If the whole leg swings backward, the hip has taken over and the hamstring is being used as
a hip extensor instead — still useful, but a different exercise from the one described here.

The slow return matters for the same reason it does everywhere else in this library: the
lengthening-under-load half is the harder and more transferable one.
