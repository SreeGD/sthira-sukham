---
anatomicalName: Adductor group (longus, brevis, magnus, gracilis, pectineus)
commonName: The inner thigh muscles, the groin muscles
abbreviations: []
region: hip
group: Hip
isContractile: true
diagramZone: thigh-inner
order: 52
roleInKneeMotion: Mostly none directly. Gracilis is the exception — it crosses the knee and
  helps bend and inwardly rotate it. The group's main influence is on how the thigh is
  positioned.
stiffnessContribution:
  whenTight: Restricts how far the leg can move out to the side, and pulls the thigh inward so the
    knee tends to drift toward the midline under load.
  whenWeak: Reduces control of the thigh in side-to-side movement and single-leg stability.
  whenInhibited: Common after groin strain, which is notoriously slow to settle.
plainLanguageGloss: The group of muscles along your inner thigh that pull your legs together.
sources:
  - neumann-kinesiology-2016
illustration:
  file: gerrish-1902-fig-356.png
  alt: Line drawing of the adductor magnus, the largest inner-thigh muscle, with its attachment
    areas shaded on the pelvis and thigh bone.
  caption: "Adductor magnus, the largest of the adductor group: outline and attachment areas."
  credit: Frederick Henry Gerrish, Text-book of Anatomy
  year: 1902
  licence: Public domain
  sourceUrl: https://commons.wikimedia.org/wiki/File:Gerrish%27s_Text-book_of_Anatomy_(1902)_-_Fig._356.png
jointInfluences:
  - joint: hip
    action: direct
    presentsAs: Restricts how far the leg can move out to the side, and limits the outward rotation
      that cross-legged sitting depends on.
  - joint: knee
    action: indirect
    presentsAs: Mostly does not cross the knee. Tightness draws the thigh inward so the knee
      collapses toward the midline under load, loading its inner side unevenly.
---

Five muscles running along the inner thigh, of which only gracilis actually reaches past the
knee. The group nonetheless matters here because of where it puts the thigh.

Their relevance to knee stiffness is positional. Squatting and deep knee bending require the
thighs to travel outward as well as the knees to fold. Adductors that will not lengthen make
that unavailable, and the movement gets achieved by letting the knees fall inward instead —
which is felt as knee stiffness, and is really hip restriction.
