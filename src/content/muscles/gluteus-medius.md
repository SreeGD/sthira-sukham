---
anatomicalName: Gluteus medius
commonName: The side-of-hip muscle
abbreviations:
  - glute med
region: hip
group: Hip
isContractile: true
diagramZone: hip-side
order: 54
roleInKneeMotion: No direct action. It holds the pelvis level in single-leg stance and controls
  inward rotation of the thigh — which determines what the knee is sitting on top of.
stiffnessContribution:
  whenTight: Uncommon, and rarely the limiting factor.
  whenWeak: The pelvis drops on the unsupported side and the thigh rolls inward every time you
    take a step or stand on one leg. The knee is carried along with it.
  whenInhibited: Very common, and frequently missed because the muscle tests strong lying down
    while failing to fire during actual weight-bearing.
plainLanguageGloss: The muscle on the side of your hip that stops your pelvis dropping when you stand on one leg.
sources:
  - neumann-kinesiology-2016
illustration:
  file: gerrish-1902-fig-352.webp
  alt: Line drawing of the gluteus medius on the side of the hip, with its attachment areas shaded
    on the pelvis.
  caption: "Gluteus medius: outline and attachment areas."
  credit: Frederick Henry Gerrish, Text-book of Anatomy
  year: 1902
  licence: Public domain
  sourceUrl: https://commons.wikimedia.org/wiki/File:Gerrish%27s_Text-book_of_Anatomy_(1902)_-_Fig._352.png
jointInfluences:
  - joint: hip
    action: direct
    presentsAs: Weakness lets the pelvis drop on the unsupported side in single-leg stance, which is
      most of walking.
  - joint: knee
    action: indirect
    presentsAs: Does not cross the knee. It controls whether the thigh bone rotates inward under
      load; when it does not, the groove the kneecap runs in rotates underneath it — the kneecap
      has not moved, but its track has.
---

If one muscle explains why a knee reference spends this much time on the hip, it is this one.

Gluteus medius holds the pelvis level when you are on one leg — which, during walking, is
most of the time. When it is not working, the pelvis drops and the thigh bone rotates
inward on every step.

The knee is attached to the top of that rotation. The kneecap groove is cut into the femur,
so if the femur rotates, the groove rotates with it while the kneecap is held in place by
the quadriceps above. The relationship between kneecap and groove changes without the
kneecap having moved at all. This is the mechanism behind a great deal of front-of-knee
stiffness, and it is the reason so much of the strength work in this library targets the hip.
