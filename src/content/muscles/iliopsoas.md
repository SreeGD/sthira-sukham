---
anatomicalName: Iliopsoas (psoas major and iliacus)
commonName: The hip flexor
abbreviations: []
region: hip
group: Hip
isContractile: true
diagramZone: hip-front
order: 56
roleInKneeMotion: No direct action on the knee. It is the main hip flexor, and it determines
  whether the hip can straighten fully — which changes what the knee has to do.
stiffnessContribution:
  whenTight: The hip cannot straighten fully, so you stand and walk with it slightly bent. The
    pelvis tips forward and the knee stays slightly bent to compensate.
  whenWeak: Reduces the ability to lift the thigh, affecting stair climbing and the swing phase of
    walking.
  whenInhibited: Less commonly inhibited than shortened; sitting keeps it short rather than quiet.
plainLanguageGloss: The deep muscle running from your lower spine and pelvis to the top of your
  thigh bone, which lifts your knee toward your chest.
sources:
  - neumann-kinesiology-2016
illustration:
  file: gerrish-1902-fig-346.png
  alt: Line drawing of the psoas major running from the lower spine to the top of the thigh bone,
    with its attachment areas shaded.
  caption: "Psoas major, the larger half of iliopsoas: outline and attachment areas."
  credit: Frederick Henry Gerrish, Text-book of Anatomy
  year: 1902
  licence: Public domain
  sourceUrl: https://commons.wikimedia.org/wiki/File:Gerrish%27s_Text-book_of_Anatomy_(1902)_-_Fig._346.png
jointInfluences:
  - joint: hip
    action: direct
    presentsAs: Tightness prevents the hip straightening fully, so you stand and walk with it
      slightly bent and the pelvis tipped forward.
  - joint: knee
    action: indirect
    presentsAs: Does not cross the knee. A hip that will not straighten leaves you standing with a
      slightly bent knee all day, so the joint never visits full extension and the quadriceps
      works continuously.
---

The primary casualty of sitting, and the clearest illustration of why a stiff knee is often a
problem somewhere else.

Sitting holds the hip flexed for hours at a time. Iliopsoas adapts to that as its resting
length, and full hip extension becomes unavailable — not through damage, but through disuse.

The knock-on is what matters here. Standing with a hip that will not fully straighten means
standing with a slightly bent knee, because that is what keeps you balanced over the foot.
The knee then spends its entire day in the middle of its range, never reaching either end,
with the quadriceps working continuously to stop it buckling. The knee feels tight and tired.
Work on the knee and little changes, because the knee is not where the problem started.
